...
test("generator prompt delegates the goal instead of performing it", () => { const prompt = generatorPrompt("Analyze the repository", { maxAgents: 10, maxConcurrency: 2, maxRounds: 2, maxTokens: 1000, timeoutMs: 1000 }); assert.match(prompt, /DO NOT perform the user's goal/); assert.match(prompt, /limits\.maxAgents must cover/); assert.match(prompt, /output only the campaign program/); });
