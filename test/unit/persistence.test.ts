...
test("append, snapshot, and replay are deterministic", async () => { const dir = await mkdtemp(join(tmpdir(), "campaign-store-")); try { const store = await EventStore.create(dir, "run", "goal", "/tmp"); const observed: string[] = []; store.subscribe((state) => observed.push(state.status)); const events = [await store.append("run.created", { summary: "An English plan summary." }), await store.append("run.started"), await store.append("node.scheduled", { nodeId: "x", attempt: 1 }), await store.append("node.completed", { nodeId: "x", output: { b: 2, a: 1 } }), await store.append("run.completed")]; const replayed = replay(initialState("run", "goal", "/tmp", store.state.createdAt), events); assert.deepEqual(replayed, store.state); assert.equal(store.state.summary, "An English plan summary."); assert.equal(observed.at(-1), "completed"); const reopened = await EventStore.open(dir); assert.deepEqual(reopened.store.state, store.state); } finally { await rm(dir, { recursive: true, force: true }); } });
