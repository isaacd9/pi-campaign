...
test("fake kernel executes sequence, bounded map, gate, checkpoint, and emit", async () => { const cwd = await mkdtemp(join(tmpdir(), "campaign-supervisor-")); try { const { ir } = compileCampaign(validCampaign); const store = await EventStore.create(join(cwd, "run"), "run", "goal", cwd); await store.append("run.created", { ir }); const kernel = new FakeKernel((assignment) => assignment.agent === "scout" ? { files: ["a.ts", "b.ts", "c.ts"] } : { edited: assignment.task }); const emitted: string[] = []; const supervisor = new CampaignSupervisor(ir, store, kernel, new GateExecutor(cwd), { pollMs: 1, emit: (message) => emitted.push(message) }); const state = await supervisor.run(); assert.equal(state.status, "completed"); assert.equal(kernel.assignments.length, 4); assert.equal(kernel.assignments[0]?.acceptance?.level, "none"); assert.deepEqual(state.outputs.map, [{ edited: "Edit a.ts" }, { edited: "Edit b.ts" }, { edited: "Edit c.ts" }]); assert.ok(state.checkpoints.includes("done")); assert.deepEqual(emitted, ["Done"]); assert.equal(state.gates[0]?.outcome, "passed"); } finally { await rm(cwd, { recursive: true, force: true }); } });
...
test("spawn uncertainty leaves a retryable interrupted node", async () => {
  ...
  try {
    ...
    kernel.spawn = async () => { throw new Error("Kernel timeout after possible launch"); };
