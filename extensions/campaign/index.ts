...
export default async function campaignExtension(pi: ExtensionAPI) {
  ...
  pi.on("input", async (event, ctx) => {
    ...
    await service.startGoal(goal, ctx);
    return { action: "handled" as const };
  });
  pi.registerCommand("campaign", { description: "Generate and run a deterministic campaign", handler: async (args, ctx) => { const goal = args.trim(); if (!goal) { ctx.ui.notify("Usage: /campaign <goal>", "warning"); return; } await service.startGoal(goal, ctx); } });
  ...
  pi.registerCommand("campaign-inspect", { description: "Open the Campaign workspace", handler: async (args, ctx) => {
    const states = await service.list(ctx);
    let runId = args.trim();
    if (!runId && states.length) {
      if (ctx.mode === "tui") {
        const choices = states.map((state) => ({ state, label: inspectChoice(state) }));
        const selected = await ctx.ui.select("Open campaign workspace", choices.map((choice) => choice.label));
        runId = choices.find((choice) => choice.label === selected)?.state.runId ?? "";
      } else runId = states[0]!.runId;
    }
    if (!runId) { ctx.ui.notify("No campaign selected.", "warning"); return; }
    await openCampaignWorkspace(service, runId, ctx);
  } });
  pi.registerCommand("campaign-run", { description: "Run a saved campaign", getArgumentCompletions: () => null, handler: async (args, ctx) => { const [name, ...rest] = args.trim().split(/\s+/); if (!name) { ctx.ui.notify("Usage: /campaign-run <name> [json-input]", "warning"); return; } try { const input = rest.length ? JSON.parse(rest.join(" ")) : undefined; await service.runSaved(name, input, ctx); } catch (error) { ctx.ui.notify(error instanceof Error ? error.message : String(error), "error"); } } });
...
function inspectChoice(state: Awaited<ReturnType<CampaignService["list"]>>[number]): string {
  const status = !state.ir && state.status === "running" ? "generating" : state.status;
  const name = state.ir?.meta.name ?? state.goal.replace(/\s+/g, " ").slice(0, 54);
  return `${status.toUpperCase()} · ${name} · ${new Date(state.updatedAt).toLocaleString()} · ${state.runId}`;
}
